...
//		RPGJS.Variables.data[0] = '需要先修 '+game.ev[2][1].name+' ！';
	...
	s(ev+'.stop()'),
	t('電學與磁學有著緊密的關係，本課程會介紹兩者間的種種密不可分關係，Maxwell’s Laws 是貫穿本學習的重要定律，想要學會電磁學，就不能不懂Maxwell’s Law。'),
	t('你要修 '+name+' 嗎？'),
	'CHOICES: ["是","否"]',
	'CHOICE_0',
		s(ev+'.is_took()'),
		'IF: "0 == variable[0]"',
			s(ev+'.can_take()'),
			'IF: "1 == variable[0]"',
				t('請問那下列哪一條並非Maxwell’s Law？'),
				'CHOICES: ["電通量密度＝介電係數×電場強度","電通量密度的散度＝單位體積電荷密度","磁通量密度的散度＝０","磁場強度的旋度＝單位面積電流密度＋單位時間內電通量密度的變化"]',
				'CHOICE_0',
					t('恭喜答對了呦～正確的應該是電場強度的旋度＝－單位時間內磁通量密度的變化'),
					s(ev+'.take()'),
				'CHOICE_1',
					s(ev+'.fail("不然你覺得什麼才是Maxwell’s Laws？再修一次會更清楚。")'),
				'CHOICE_2',
					s(ev+'.fail("不然你覺得什麼才是Maxwell’s Laws？再修一次會更清楚。")'),
				'CHOICE_3',
					s(ev+'.fail("不然你覺得什麼才是Maxwell’s Laws？再修一次會更清楚。")'),
